...
export default function Operations() {
  ...
  return (
    <>
      ...
      {data.inventory.length === 0 && data.orders.length === 0 && data.batches.length === 0 && (
        <div style={{ padding: '2rem', borderRadius: 12, border: '2px dashed var(--border)', textAlign: 'center', marginBottom: '1.5rem' }}>
          <div style={{ fontSize: 32, marginBottom: 8 }}>📦</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 6 }}>No operations data yet</div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', maxWidth: 380, margin: '0 auto 16px' }}>
            Add your SKUs, production batches, and fulfillment orders to track your supply chain.
          </div>
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
            <button onClick={() => setInvModal({})} style={{ padding: '8px 18px', background: 'var(--navy)', color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
              + Add SKU
            </button>
            <button onClick={() => setBatchModal({})} style={{ padding: '8px 18px', background: 'transparent', color: 'var(--navy)', borderRadius: 8, fontSize: 13, fontWeight: 700, border: '1px solid var(--border)', cursor: 'pointer' }}>
              + Create Batch
            </button>
          </div>
        </div>
